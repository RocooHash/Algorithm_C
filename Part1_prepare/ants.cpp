...
These two numbers are followed by n integers giving the position of each ant on the pole as the distance measured from the left end of the pole, 
in no particular order. All input integers are not bigger than 1000000 and they are separated by whitespace.
Output
...
(if the directions of their walks are chosen appropriately) and the second number is the latest possible such time.
Sample Input
...
int main()
{
    ...
    while (caseNum--)
    {
        ...
        for(int j =0;j<n;j++){
            cin>>a[j];
            minT = max(minT, min(a[j], l-a[j]));
            maxT = max(maxT, max(a[j], l-a[j]));
        }
        cout<<minT<<" "<<maxT<<endl;
    }
}